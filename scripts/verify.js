...
const variants = ['baseline', 'before', 'before-replace', 'before-sms', 'builtin-devtool', 'after-good', 'after-bad']
const checks = [
...
async function main() {
  ...
  for (let i = 0; i < checks.length; i++) {
    ...
    const builtinDevtool = results['builtin-devtool'][i]
    ...
    assert(builtinDevtool.generatedLine === baseline.generatedLine + 1, `${builtinDevtool.label}: builtin-devtool build should shift generated line by 1`)
    ...
    assert(builtinDevtool.originalLineFromMap === builtinDevtool.originalLine, `${builtinDevtool.label}: builtin-devtool build should map back to the original source line`)
